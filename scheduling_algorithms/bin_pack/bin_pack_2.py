...
class BinPack:
    ...
    def run(self):
        ...
        sorted_workers = sorted(self.workers, key=lambda x: x.cpu.cores - x.cpu.cores_used, reverse=False)
        ...
        sorted_has = sorted(has, key=lambda x: x[2], reverse=True)

        unique_tasks = []

        for u, v, w in sorted_has:
